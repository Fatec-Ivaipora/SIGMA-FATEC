...
// pontuados.
const CRITERIOS: { key: keyof NotasCriterios; label: string }[] = [
...
export default function AvaliadorTrabalhosPage() {
  ...
  const [notas, setNotas] = useState<Partial<Record<keyof NotasCriterios, number>>>({});
  ...
  async function enviarNota(id: string) {
    ...
    const notasCriterios = notas as NotasCriterios;
    await atualizarTrabalho(id, {
      status: "avaliado",
      notasCriterios,
      notaAvaliador: somaNotas,
  ...
  async function enviarRevisao(id: string) {
    if (!comentarioRevisao.trim()) return;
    await atualizarTrabalho(id, {
      status: "revisao",
      comentarioRevisao: comentarioRevisao.trim(),
      atualizadoEm: serverTimestamp(),
    });
    setAvaliando(null);
    setPedindoRevisao(false);
      ...
      <div className="flex flex-1 flex-col overflow-x-hidden">
        <header className="border-b border-fatec-line bg-white px-6 py-5 md:px-10">
          ...
          <p className="text-sm text-fatec-muted">
            Trabalhos que a organização enviou para você.
          </p>
        </header>
          ...
          <div className="flex flex-col gap-4">
            ...
            {trabalhosFiltrados.length === 0 && (
              <p className="rounded-2xl border border-dashed border-fatec-line bg-white px-6 py-10 text-center text-sm text-fatec-muted">
                Nenhum trabalho designado a você ainda.
              </p>
            )}
                      ...
                      <>
                        <div className="flex flex-col gap-3">
                          <span className="text-sm font-medium text-fatec-navy-900">
                            Critérios de avaliação (1 a 5 cada — a nota final é
                            a soma)
                          </span>
                          {CRITERIOS.map((c) => (
                            <div
                            ...
                            >
                              <span className="text-sm text-fatec-ink">
                                {c.label}
                              </span>
                          ...
                          <p className="text-sm text-fatec-muted">
                            Total:{" "}
                            <span className="font-semibold text-fatec-navy-900">
                              {somaNotas}
                            </span>{" "}
                            / 25
                          </p>
                      ...
                      <>
                        <label className="flex flex-col gap-1.5">
                          ...
                          <textarea
                            ...
                            onChange={(e) =>
                              setComentarioRevisao(e.target.value)
                            }
